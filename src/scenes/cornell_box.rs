...
pub fn build(width: usize, height: usize) -> Scene {
	...
	let floor = Quad::new(Color::WHITE, front_right_bottom, front_left_bottom, back_left_bottom, back_right_bottom);
	scene.entities.push(Box::new(floor));
	let ceiling = Quad::new(Color::WHITE, front_left_top, front_right_top, back_right_top, back_left_top);
	scene.entities.push(Box::new(ceiling));
	let back_wall = Quad::new(Color::WHITE, back_left_bottom, back_left_top, back_right_top, back_right_bottom);
	scene.entities.push(Box::new(back_wall));
	let front_wall = Quad::new(Color::WHITE, front_left_bottom, front_right_bottom, front_right_top, front_left_top);
	scene.entities.push(Box::new(front_wall));
	let left_wall = Quad::new(Color::RED, front_left_bottom, front_left_top, back_left_top, back_left_bottom);
	scene.entities.push(Box::new(left_wall));
	let right_wall = Quad::new(Color::GREEN, front_right_bottom, back_right_bottom, back_right_top, front_right_top);
